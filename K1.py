...
def reg():
    ...
    try:
        ...
        reg2()
    r = requests.get('https://github.com/Badshawazir/K1/blob/main/Approvel.txt').text
    if to in r:
        time.sleep(2)
        bsn_menu()
    else:
...
def chigozie():
    ...
    if cookie in['OPEN','Open','open']:
      jalan("\n  %s* %sanda akan di arahkan ke YouTube"%(O,O));time.sleep(3);os.system('xdg-open https://wa.me/+923345478919');chigozie()
    try:
        ...
        os.system('xdg-open https://wa.me/+923345478919')
        bsn_menu()
